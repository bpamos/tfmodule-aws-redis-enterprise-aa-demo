...
output "crdb_cluster1_redis_cli_cmd" {
  value = format("redis-cli -h redis-%s.%s -p %s -a %s", var.crdb_port,var.dns_fqdn1,var.crdb_port,var.crdb_db_password)
}

output "crdb_cluster2_redis_cli_cmd" {
  value = format("redis-cli -h redis-%s.%s -p %s -a %s", var.crdb_port,var.dns_fqdn2,var.crdb_port,var.crdb_db_password)
}